...
| 版本 | 日期 | 类型 | 核心变化 | 兼容性 |
|---|---|---|---|---|
| v2.0.0 | 2026-09-17 | 主版本 | 默认直接审查；HF 规则按类型分片；PX/SPT、COPY 与 UC 改为显式加载；新增 token 预算 CI。 | 不兼容；依赖旧自动确认、自动 SPT、自动清单或自动 UC 的调用方需显式声明。 |
...
### v2.0.0 · 按需加载与 token 预算

**变更类型。** 主版本。

**变更原因。** v1 的常见页面审查路径会加载单体 HF 规则库、维度确认、范围配置、SPT、修改清单和规范缺口说明，参考上下文可达到约 1.0 万至 2.0 万 token；普通请求还会先产生一次 A–F 确认轮次和重复的报告/清单输出。

**新增或修改内容。** 将 `SKILL.md` 收敛为边界、默认值、一级路由、六步流程和最小输出契约；将 75 个既有 `HF-` 规则按核心、标点、格式、状态反馈、术语组件和语言拆为六个分片，规则编号保持不变；移除维度确认与复杂配置协议；将一般反馈留在 HF，只有显式指定系统产品提示信息规范时加载 SPT；将修改清单和规范缺口改为显式触发，并用紧凑 schema 替代重复模板。新增 `evals/token-budgets.json`、`scripts/validate_token_budget.py` 与 CI 预算检查，更新回归用例覆盖默认直出、一般反馈不加载 SPT、显式 SPT、按需 COPY 和按需 UC。

**行为变化。** 用户只说“审查/看看页面”时直接执行，不再展示 A–F 菜单。没有明确要求时不生成独立 Markdown 修改清单、不检查 UC 规范缺口、不加载 SPT 来源规则，P3 默认省略。用户仍可通过自然语言显式启用这些能力。

**受影响文件。** `SKILL.md`、`README.md`、`CONTRIBUTING.md`、`docs/README.md`、`docs/VERSION_HISTORY.md`、`.github/workflows/validate-skill.yml`；新增六个 `references/hf-*.md`、`change-list-schema.md`、`coverage-gap-schema.md`，修改 PX/SPT 与 Prompt 资源，删除五个 v1 运行文件；新增或修改 `scripts/validate_skill_consistency.py`、`scripts/validate_token_budget.py`、`evals/evals.json` 与 `evals/token-budgets.json`。`references/source-register.md` 和 `evals/trigger-eval.json` 内容未修改。

**兼容性与迁移。** 不兼容。依赖旧自动行为的调用方应显式加入“生成 Markdown 修改清单”“按 SPT 规范”“识别规范缺口”或“包含 P3”。规则消费者可按前缀将旧单体 HF 文件映射至新的六个分片，全部 75 个规则 ID 均保留。

**来源与边界。** 不新增外部来源，不改变 PX/SPT 已提炼内容。`source-register.md` 改为只在来源核验、冲突和维护时加载；来源规则文件仍保留最小元数据。

**校验状态。** 已通过严格结构校验（含 75 个 HF ID、6 条回归语义契约和 12 条必需 token 路径）、Skill Creator 基础校验、12 条 token 预算、JSON/正则校验、7 类负向校验器测试与 `git diff --check`。六条行为回归全部通过；修正 P3 默认关闭后定向复测通过；触发集抽样 12 条正负用例全部匹配。
