...
| 用户发送一至五条明确 UI 文案，并询问“是否合规/有没有问题/怎么改” | 本文件；按文案类型读取对应 `HF-`、`SPT-` 或已确认的 `PX-` 资源 | 使用单条文案直接问答模式：最小回答，不生成完整报告或修改清单。 |
...
当用户一次提供 **一至五条可读、明确的 UI 文案**，并直接询问“是否符合规范”“有没有问题”“这样写可以吗”或“怎么改”时，使用此模式；该请求已构成明确范围，**不要**先展示 A–F 维度菜单。若用户提供页面/流程材料、要求完整审查或修改清单、要求跨页面比对，或文本超过五条且未明确要求简答，则进入完整审查流程。

1. 将 `core` 设为 `on`；将 `cross_page`、`coverage_gap` 与 `change_list` 设为 `off`。仅在用户明确指定设计系统时加载 `PX-`；仅在文案属于成功、失败、错误、异常、进行中或风险提示时按需加载 `SPT-`。
2. 只检查当前可见文本可判定的书写、格式、术语、组件角色或提示信息结构。缺少组件、对象、真实状态、业务口径或来源依据时，输出“待确认”，不要推断业务含义。
3. 对每条文案直接输出不超过三项：**结论**（符合 / 不符合 / 待确认）、**依据**（实际适用的规则编号或“当前范围内未发现可验证问题”）与**建议**（保持原文、最小改写或说明需补充的信息）。
4. 不输出 P0–P3、完整审查范围表、`COPY-` 编号、独立 Markdown 修改清单或 `UC-` 治理表。若一次提供多条，按原顺序逐条回答；只有用户追问页面或流程一致性时才升级为完整审查。
...
单条文案直接问答模式按“符合 / 不符合 / 待确认”给出最小答案，不使用以下 P0–P3 分级或完整报告字段。其他任务按完整审查流程执行。
