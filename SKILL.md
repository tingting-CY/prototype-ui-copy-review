...
description: 审查原型、线框图、页面流程、界面截图、录屏、文案清单或需求文档中的简体中文 UI 文案（标题、导航、字段、占位符、按钮、弹窗、空状态、成功/失败提示与风险提示），依据 HF-（通用高频规则）、PX-（PaletX/ZTE 设计系统）、SPT-（系统提示信息句型）定位术语、命名、标点、格式、状态、反馈与跨页面一致性问题，输出带规则编号的 P0–P3 审查报告、可勾选的 COPY- Markdown 修改清单，并单列未被规范覆盖的 UC- 场景。只要用户要求“看看/检查/check/审查/把关/review”某个页面、截图、原型或一批界面文字，或提到文案规范、文案一致性、术语统一、按钮/弹窗/提示语措辞、提示信息规范、PaletX、ZTE 设计系统、设计评审文案问题、给开发直接替换的文案清单，就使用本 Skill，哪怕用户没说“审查”二字、只贴了一张图、材料还没附上，或只要求检查其中一类问题（例如只看标点和状态词、只统一几个近义词、只把泛化按钮词改成具体动作）。不用于翻译或多语言本地化、营销长文案写作、视觉与交互走查、代码中的 i18n 资源整理。
---
...
只基于可见文字、结构化材料和已确认规则下结论。审查结论会被设计、产品和开发直接拿去改稿，一条猜出来的“问题”足以让整份清单失去可信度，因此：不要猜测模糊、截断、未提供或无法进入的内容；不要把视觉、交互、功能完整性或未证实的业务规则当作文案缺陷。业务定义、法务/安全口径、技术参数或品牌术语不明确时，标记为“待确认”或“规范待维护项”。
...
| 维护本 Skill（改规则、改流程、发版），而非执行审查 | [`CONTRIBUTING.md`](CONTRIBUTING.md)；[`docs/README.md`](docs/README.md)；[`docs/VERSION_HISTORY.md`](docs/VERSION_HISTORY.md)；[`scripts/validate_skill_consistency.py`](scripts/validate_skill_consistency.py)；[`evals/evals.json`](evals/evals.json) | 贡献规则、用户手册、版本记录、结构与敏感信息校验、回归用例。审查任务不需要读取。 |
...
1. **界定范围并确认维度。** 记录材料类型、已覆盖页面/状态、可见文字和限制。用户已指定规则、维度、排除项或“完整审查”时直接解析配置；仅说“审查/检查”时，按 `review-dimension-confirmation.md` 追问一次——不熟悉规则模块的用户通常不知道可以选什么，先问一次比事后删掉一批不适用的 PX/SPT 结论成本更低。用户回复选项、自然语言范围或“按推荐执行”后再继续。
...
单条问题的写法示例（审查报告与修改清单共用这组字段；完整模板见 `markdown-change-list.md`）：

```markdown
- [ ] **COPY-003｜成员管理 · 删除确认弹窗**
  - **原文：** 确定删除？／按钮“确定”
  - **修改为：** 删除成员“张三”后不可恢复。是否删除？／按钮“删除成员”
  - **说明：** 高影响操作未说明对象与后果；泛化按钮无法预期结果。
  - **依据：** HF-CMP-09、HF-STA-06
  - **影响范围：** 成员管理列表页、成员详情页的删除入口
```
...
修改 `SKILL.md`、`references/`、来源登记、提示词、输出行为、`scripts/` 或 `docs/` 时，按 [`CONTRIBUTING.md`](CONTRIBUTING.md) 同步用户文档与版本记录，依次运行 `python3 scripts/validate_skill_consistency.py --strict` 与 Skill 结构校验，并用 `evals/evals.json` 做回归。不要在本文件重复规则库、提示词模板、修改清单模板、来源细节或完整版本历史；以上资源各自只有一个维护位置。
